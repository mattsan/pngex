...
defmodule TestPixel do
  ...
  def get_grayscale(%{width: width, height: height, depth: depth}, n)
      when depth in [1, 2, 4, 8] do
  ...
  def get_alpha(%{width: width, height: height, depth: depth}, n) when depth in [8, 16] do
    x = rem(n, width) / width
